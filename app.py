...
# --- Load model: try bundled file first, fall back to upload ---
model = None
try:
    ...
    st.warning(
        ...
        "Upload it below (it's the file your notebook creates at the end with `final_model.save_model(...)`)."
    )
    uploaded_model = st.file_uploader("Upload catboost_model.cbm", type=["cbm"])
